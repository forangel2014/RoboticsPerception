function [ warped_pts ] = warp_pts( video_pts, logo_pts, sample_pts,logox,logoy)
...
% YOUR CODE HERE
[n,~] = size(sample_pts);
sample_pts = [sample_pts'; ones(1,n)];
warped_pts = H*sample_pts;
warped_pts = warped_pts(1:2,1:n)';
warped_pts(warped_pts < 1) = 1;
warped_pts(warped_pts(:,1) > logox, 1) = logox;
warped_pts(warped_pts(:,2) > logoy, 2) = logoy;
end
